...
export default function NewsPage() {
  ...
  const publishedThisWeek = news.filter(
    (item) => item.createdAt && isThisWeek(new Date(item.createdAt))
  ).length;
  const publishedThisMonth = news.filter(
    (item) => item.createdAt && isThisMonth(new Date(item.createdAt))
  ).length;
  const uncategorizedArticles = news.filter((item) => !item.category).length;
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{uncategorizedArticles}</div>
              <p className="text-xs text-muted-foreground">
                {t("news.noCategoryArticles")}
              </p>
            </CardContent>
          </Card>
        </div>
